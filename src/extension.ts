...
export function activate(context: vscode.ExtensionContext) {
  ...
  vscode.window.createTreeView('mcpWorkspaceConfigs', { 
  ...
  // Refresh command
  const refreshCommand = vscode.commands.registerCommand('mcpManager.refreshConfigs', () => {
    configProvider.refresh();
    workspaceProvider.refresh();
    vscode.window.showInformationMessage('MCP configurations refreshed!');
  });

  // Add server command
  const addServerCommand = vscode.commands.registerCommand('mcpManager.addServer', () => {
    vscode.commands.executeCommand('mcpManager.openManager');
  });

  // Edit server command
  const editServerCommand = vscode.commands.registerCommand('mcpManager.editServer', (item: MCPServerItem) => {
...
class MCPConfigTreeDataProvider implements vscode.TreeDataProvider<MCPTreeItem> {
  ...
  getChildren(element?: MCPTreeItem): Thenable<MCPTreeItem[]> {
    ...
    } else if (element instanceof MCPConfigItem) {
      return Promise.resolve(this.getServers(element.configPath));
    }
  ...
  private getMCPConfigs(): MCPTreeItem[] {
    ...
    if (workspaceFolders) {
      const commonPaths = [
        { path: '.mcp.json', label: 'Workspace MCP Config' },
        { path: 'mcp.json', label: 'Project MCP Config' },
        { path: '.claude/mcp.json', label: 'Claude MCP Config' },
        { path: 'config/mcp.json', label: 'Config MCP File' }
      ];
      
      for (const folder of workspaceFolders) {
        for (const { path: commonPath, label } of commonPaths) {
          const fullPath = path.join(folder.uri.fsPath, commonPath);
          if (fs.existsSync(fullPath)) {
            configs.push(new MCPConfigItem(
              label,
              fullPath,
  ...
  private getServers(configPath: string): MCPTreeItem[] {
...
class MCPWorkspaceTreeDataProvider implements vscode.TreeDataProvider<MCPTreeItem> {
  private _onDidChangeTreeData: vscode.EventEmitter<MCPTreeItem | undefined | null | void> = new vscode.EventEmitter<MCPTreeItem | undefined | null | void>();
  readonly onDidChangeTreeData: vscode.Event<MCPTreeItem | undefined | null | void> = this._onDidChangeTreeData.event;
  ...
  getChildren(element?: MCPTreeItem): Thenable<MCPTreeItem[]> {
    if (!element) {
      return Promise.resolve(this.getWorkspaceConfigs());
    }
    return Promise.resolve([]);
  }

  private getWorkspaceConfigs(): MCPTreeItem[] {
    const configs: MCPTreeItem[] = [];
    const workspaceFolders = vscode.workspace.workspaceFolders;
      ...
      for (const folder of workspaceFolders) {
        ...
        jsonFiles.forEach(file => {
          if (this.isMCPConfig(file)) {
            configs.push(new MCPWorkspaceItem(
              path.basename(file),
              file,
              vscode.TreeItemCollapsibleState.None
            ));
  ...
  private isMCPConfig(filePath: string): boolean {
    try {
...
class MCPConfigItem extends MCPTreeItem {
  constructor(
    label: string,
    ...
    this.contextValue = 'mcpConfig';
    this.iconPath = new vscode.ThemeIcon('json');
  }
}

class MCPServerItem extends MCPTreeItem {
  constructor(
    label: string,
    ...
    super(label, collapsibleState);
    this.tooltip = `Command: ${server.command.join(' ')}`;
    this.description = server.command[0];
    this.contextValue = 'mcpServer';
    this.iconPath = new vscode.ThemeIcon('server-environment');
    
    this.command = {
      command: 'mcpManager.editServer',
      title: 'Edit Server',
      arguments: [this]
    };
  }
...
class MCPWorkspaceItem extends MCPTreeItem {
  constructor(
    label: string,
    ...
    this.contextValue = 'mcpWorkspaceConfig';
